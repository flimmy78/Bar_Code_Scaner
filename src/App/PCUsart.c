...
unsigned char g_com_handle_flag;					//串口交互标志
unsigned int		vcom_device_state;

extern unsigned char g_usb_type;

TPCCommand			g_pcCommand;
...
void USB_Cable_Config (unsigned char NewState)
{
	...
	gpio_init.GPIO_Pin = USB_DISCONNECT_PIN;
	gpio_init.GPIO_Speed = GPIO_Speed_10MHz;
	gpio_init.GPIO_Mode = GPIO_Mode_Out_OD;
	GPIO_Init(USB_DISCONNECT, &gpio_init);
